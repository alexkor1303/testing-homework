...
## Все баги ловятся,но на некоторых прерывается выполнение тестов из-за чего последующие автоматически становятся не пройденными.

### UNIT TESTS : 8

### TESTPLANE : 18

#### Что бы запустить TESTPLANE тесты с нужным BUG_ID :

#### Проверьте работу тестов без багов командой npx testplane.

#### Далее перейдите в папку test -> urlCreator -> в константу bug_id введите номер бага для теста и запускайте тесты обычной командой npx testplane (необходимые url адреса сформируются автоматически)
